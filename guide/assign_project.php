...
$query2 = "SELECT p.id, p.project_name FROM project_details p LEFT JOIN group_allot g ON p.id = g.project_id WHERE p.id NOT IN(select project_id from group_allot)";
$result2 = $mysqli->query($query2) or die($query2."<br>".$mysqli->error);
//Select group_id from Database
$query3 = "SELECT * FROM project_group p INNER JOIN users u on p.user_id = u.id inner join group_allot g on g.leader_id = u.id WHERE p.is_leader = 1 AND g.guide_id = $user_id";
        ...
        <i class="fa fa-bars"></i> </a> 
          ...
          <a class="btn btn-link visible-xs" data-toggle="dropdown" data-target=".user"> <i class="fa fa-cog"></i> </a> </div> 
             ...
             <span class="thumb-sm avatar pull-left"> <img src="../images/profile/<?php echo $_SESSION['profile']?>" alt="..."> 
              ...
              </span> <li> <a href="../logout.php">Logout</a> 
                 </li>
                  </ul> 
                  </li>
                  </ul>
                                  ...
                                  <div class="col-sm-6 col-sm-offset-2"> 
                                    ...
                                    <label>Select Group Leader</label>
                                      <select class="form-control" id="leaderid">
                                        <?php while($data1 = $result3->fetch_assoc()){?>
                                        <option value="<?php echo $data1['user_id']?>"><?php echo $data1['firstname']." ".$data1['lastname'];?></option>
                                        <?php }?>
                                      </select>
                                    </div>
                                    <div class="form-group col-sm-2"><br>
                                      <p><button class="btn btn-warning" id="allot_project">Assign</button></p>
                                    </div>
                                    <h1 id="result"></h1>
                                  </div>
                             </div>
