...
$query2 = "SELECT u.id, u.firstname, u.lastname FROM users u LEFT JOIN project_group p on u.id = p.id WHERE u.id NOT IN (select user_id from project_group) and u.user_type = 3";
$result2 = $mysqli->query($query2) or die($query2."<br>".$mysqli->error);
//Select group_id from Database
$query3 = "SELECT * FROM project_group p INNER JOIN users u on p.user_id = u.id WHERE p.is_leader = 1";
                        ...
                        <li class="divider"></li> 
                          ...
                          <li><a href="reference.php" class="auto"> <i class="i i-statistics icon"> </i> <span class="font-bold">Reference</span> </a> 
                            ...
                            <a href="#nav" data-toggle="class:nav-xs" class="btn btn-icon icon-muted btn-inactive m-l-xs m-r-xs"> <i class="i i-circleleft text"></i> <i class="i i-circleright text-active"></i> </a> </footer>
                             ...
                             <section id="content"> 
                             <section class="hbox stretch"> <section> <section class="vbox"> <section class="scrollable padder"><section class="row m-b-md"> <div class="col-sm-6"> <h3 class="m-b-xs text-white">Allot student</h3> <small>Welcome back, <?php echo $_SESSION['username']?>, <?php
                                  ...
                                  <div class="col-sm-6 col-sm-offset-2"> 
                                    ...
                                    <label>Select Group Leader</label>
                                      <select class="form-control" id="group">
                                        ...
                                        <option value="<?php echo $data['group_id']?>"><?php echo $data['firstname']." ".$data['lastname'];?></option>
                                        <?php }?>
                                      </select>
                                    </div>
                                    <div class="form-group col-sm-2"><br>
                                      <p><button class="btn btn-warning" id="allot_student">Assign</button></p>
                                    </div>
                                    <h1 id="result"></h1>
                                  </div>
                             </div>
